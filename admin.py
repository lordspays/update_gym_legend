...
async def admin_main_handler(message: Message):
    ...
    main_commands = (
        ...
        "• Связь [айди] [сообщение] - отправить личное сообщение\n\n"
...
async def broadcast_message_handler(message: Message, cmd_args: str):
    ...
    return (
        ...
        f"📊 Статистика:\n"
        f"♦️ Всего игроков: {total_players}\n"
        f"♦️ Успешно отправлено: {successful_sends}\n"
        f"♦️ Не удалось отправить: {failed_sends}\n"
        f"♦️ Процент успеха: {(successful_sends/total_players*100):.1f}%\n\n"
        f"📝 Текст сообщения:\n{message_text}\n\n"
